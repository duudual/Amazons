...
int iterations = 10000;         // MCTS迭代次数
double c = 1.414;               // UCT探索参数 (≈√2)
int parallelSimulations = 8;    // 每个节点并行模拟次数
...
private:
    ...
    condition_variable completionCondition;  // 新增：用于等待任务完成
    atomic<bool> stop;
    atomic<int> activeTasks;  // 新增：跟踪活跃任务数

public:
    SimpleThreadPool(size_t numThreads) : stop(false), activeTasks(0) {
            ...
            workers.emplace_back([this, i] {
                while (true) {
                    ...
                    if (task) {
                        ...
                        completionCondition.notify_all();  // 通知任务完成
                    }
                }
            });
        }
...
struct MCTSNode {
    ...
    double getUCTValue(double c = 1.414) const {
...
// ========== 全局线程池 ==========
static SimpleThreadPool* g_threadPool = nullptr;
static mutex g_threadPoolMutex;

// 线程安全的初始化
void initThreadPool() {
    lock_guard<mutex> lock(g_threadPoolMutex);
    if (!g_threadPool) {
        int numThreads = thread::hardware_concurrency();
        if (numThreads == 0) numThreads = 8;
        g_threadPool = new SimpleThreadPool(numThreads);
    }
}
...
MCTSNode* expandNode(MCTSNode* node) {
    ...
    uniform_int_distribution<size_t> dist(0, unexpandedMoves.size() - 1);
...
// 修复：传递根玩家作为参数
double simulateGameThreadSafe(MCTSNode* node, int maxDepth, int rootPlayer, mt19937* rng) {
    ...
    mt19937* actual_rng = rng ? rng : &local_rng;

    while (steps < maxDepth) {
        GameStatus status = checkGameStatus(simBoard);
        if (status != GAME_ONGOING) {
            if (status == BLACK_WIN) {
                return (rootPlayer == grid_black) ? 1.0 : 0.0;
            } else if (status == WHITE_WIN) {
                return (rootPlayer == grid_white) ? 1.0 : 0.0;
            } else {
                return 0.5;
            }
        }
        ...
        int randomIndex = dist(*actual_rng);
    ...
    double score = quickScore(simBoard, rootPlayer);
    return (score > 0) ? 1.0 : (score < 0) ? 0.0 : 0.5;
}

// 修复：使用智能指针和条件变量
double simulateGameParallelBatch(MCTSNode* node, int maxDepth, int numSimulations, int rootPlayer) {
    if (!g_threadPool) {
        initThreadPool();
    }

    // 使用 shared_ptr 管理生命周期
    auto results = make_shared<vector<double>>(numSimulations);
    auto completed = make_shared<atomic<int>>(0);
    auto mtx = make_shared<mutex>();
    auto cv = make_shared<condition_variable>();

    for (int i = 0; i < numSimulations; i++) {
        g_threadPool->enqueue([node, maxDepth, rootPlayer, results, i, completed, cv]() {
            thread_local mt19937 rng(random_device{}());
            (*results)[i] = simulateGameThreadSafe(node, maxDepth, rootPlayer, &rng);
    ...
    cv->wait(lock, [completed, numSimulations]() {
        return completed->load() >= numSimulations;
    });
    ...
    return totalResult / numSimulations;
}
...
// 主搜索函数
Move monteCarloSearch(int color, int iterations, int maxDepth, int c) {
    initThreadPool();
    ...
    for (int i = 0; i < iterations; i++) {
        ...
        // 传递根玩家颜色
        double simulationResult = simulateGameParallelBatch(expandedNode, maxDepth, parallelSimulations, color);
